...
class SpecificWorker(GenericWorker):
    ...
    def compute(self):
        ...
        # if self.results['valid'] and self.critical_area.contains(self.person_point):
        # self.add_pose_from_dsr()

        # if self.critical_area.contains(self.person_point) and self.personin:
        #     self.carla_proxy.setSimulationParam(self.agent2interface(self.cond_env))
        #     self.personin = False
        ...
        for actor in self.actor_list:
            ...
            if actor["rol"] == "person":
                if len(actor['initPose']) > 1:
                    ...
                    self.current_speed_person = np.linalg.norm(velocity) / self.revision_time #TODO: GENERALIZACIÓN N PERSONAS CON DICCIONARIO AAAA
                    factor = self.current_speed_person/2
                    self.distance_dispersion = 2 + 2*factor
                    self.angle_dispersion = 360 - 180*factor
    def processing(self):
        x_init_person = []
        y_init_person = []
        x_final_person = []
                ...
                if (len(actor_info[i]['fullPose'])>0):
                    ...
                    try:
                        if ind < len(actor_info[i]['fullPose']):
                            ...
                            diff_alfas = np.deg2rad(alfa_carla)+np.pi - np.deg2rad(alfa_copp)+np.pi
                            if diff_alfas < 0:
                                diff_alfas = diff_alfas + 2*np.pi
                            elif diff_alfas > 2*np.pi:
                                diff_alfas = diff_alfas - 2 * np.pi
                            ...
                            if actor_info[i]['rol'] == 'person':
                                ...
                                # if error <= self.distance_dispersion and diff_alfas <= self.angle_dispersion:
                                print("DIFERENCIA ANGULOS: ", diff_alfas)
                                print("ANGLE DISPERSION: ", self.angle_dispersion)
                                if error <= self.distance_dispersion and diff_alfas <= self.angle_dispersion:
                                # if True:
                                    if ind == 0:
                                    ...
                                    x_final_person_discard.append(pose[0])
                                    y_final_person_discard.append(pose[1])
        ...
        plt.scatter(x_init_person, y_init_person, s=15, color='red')
        plt.scatter(x_final_person, y_final_person, s=15, color='blue')
        ...
        plt.scatter(x_init_car, y_init_car, s=15, color='orange')
        plt.scatter(x_final_car, y_final_car, s=15, color='green')
        plt.scatter(self.x_path, self.y_path, s=10, color='black')
        ...
        if len(total_angulo) > 0:
            #ang_to_car = self.Average(total_angulo)
            ang_to_car = np.mean(total_angulo)
            ang_to_car = ang_to_car + alfa_copp / 2
            # self.direction_to_carla = ang_to_car

            #self.std_to_car = self.angle_dispersion
            print("ANGULO SIMULACIONES: ", ang_to_car)
            carla_pose = self.world_to_carla(0, 0, np.deg2rad(ang_to_car))
            npError = np.array(processerror)
            npAng = np.array(processangulo)
            sumpesos =0
            maxe = npError.max()
            mine = npError.min()
            #aux = [(er-maxe)/(mine-maxe) for er in proceserror]
            if mine-maxe != 0:
                npPesos = (npError-maxe)/(mine-maxe)
            else:
                npPesos = np.array([1])
                print("////////////////////////////////////////////////////////////////////",angulo_y_error)

            for count, i in enumerate(angulo_y_error):
                i["peso"] = npPesos[count]/sum(npPesos)
                sumpesos += i["peso"]
            ordenados = sorted(angulo_y_error, key=lambda angulo: angulo['peso'])
            mean = np.average(npAng,weights=npPesos,axis=0)
            var = np.average((npAng-mean)**2, weights=npPesos,axis=0)

            if self.current_speed_person < 0.1:
                self.direction_to_carla = alfa_copp
                self.std_to_car = np.std(total_angulo)
            else:
                self.direction_to_carla = mean
                self.std_to_car = np.sqrt(var) + 15
