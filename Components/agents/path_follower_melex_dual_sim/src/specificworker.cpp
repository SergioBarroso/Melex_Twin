...
void SpecificWorker::initialize(int period)
{
    ...
    else
    {
        ...
        // connect(G.get(), &DSR::DSRGraph::update_attrs_signal, this, &SpecificWorker::add_or_assign_attrs_slot, Qt::QueuedConnection);
...
{
    auto &[adv_, side_, rot_] = speeds;
    float k_brake_act, k_time_act, k_percentage_act;
...
//                    std::cout<<v.value().index()<<endl;
                    ...
                    k_percentage_act = exponentialFunction(collision_percentage, 0.6, 0.4, 0);
                    if(k_percentage_act < k_percentage) k_percentage = k_percentage_act;
                    ...
                    k_time_act = 1- exponentialFunction(min_time ,10,0.4,0);
                    if(k_time_act < k_time) k_time = k_time_act;
                }
...
//                    std::cout<<v.value().index()<<endl;
                ...
                k_brake_act = exponentialFunction(brake_percentage, 0.3, 0.9, 0);
                if(k_brake_act < k_brake) k_brake = k_brake_act;
            }
        }
    }
...
{
    if(type == RT_type_name)
    {
        auto element_pos = std::find(people_ids.begin(), people_ids.end(), to);
        if(element_pos != people_ids.end())
        {
            int index = std::distance(people_ids.begin(), element_pos);
            if(auto edge_robot = rt_api->get_edge_RT(G->get_node(from).value(), to); edge_robot.has_value())
            {
                if(auto person_pos = G->get_attrib_by_name<rt_translation_att>(edge_robot.value()); person_pos.has_value())
                {
                    auto person_node_pos_val = person_pos.value().get();
                    widget_2d->scene.removeItem(people_ellipses[index]);
                    people_ellipses[index] = widget_2d->scene.addEllipse(-200, -200, 400, 400, QPen(QColor(0, index*85, 0)), QBrush(QColor(0, index*85, 0)));
                    people_ellipses[index]->setPos(person_node_pos_val[0],person_node_pos_val[1]);
                    people_ellipses[index]->setZValue(100);
                }
            }
        }
    }
...
{
    if(type == person_type_name)
    {
        if (!std::count(people_ids.begin(), people_ids.end(), id)) 
        {   
            people_ids.push_back(id);
            QGraphicsEllipseItem *target;
            if(auto edge_robot = rt_api->get_edge_RT(G->get_node("world").value(), id); edge_robot.has_value())
            {
                if(auto person_pos = G->get_attrib_by_name<rt_translation_att>(edge_robot.value()); person_pos.has_value())
                {
                    auto person_node_pos_val = person_pos.value().get();
                    // widget_2d->scene.removeItem(target);
                    target = widget_2d->scene.addEllipse(-200, -200, 400, 400, QPen(QColor("Yellow")), QBrush(QColor("Yellow")));
                    target->setPos(person_node_pos_val[0],person_node_pos_val[1]);
                    target->setZValue(100);
                }
            }
            people_ellipses.push_back(target);
            return;
        }
    }
    //check node type
